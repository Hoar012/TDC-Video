...
from tdc.mm_utils import KeywordsStoppingCriteria
...
def find_all_linear_names(model):
    ...
    if 'q_proj' in lora_module_names: # needed for 16-bit
        ...
        lora_module_names.add('self_attn.q_proj')
        
    print("Trainable lora modules:", lora_module_names)
    # {'up_proj', 'vision_proj', 'k_proj', 'gate_proj', 'query_proj', 'v_proj', 'o_proj', 'value', 'down_proj', 'self_attn.q_proj', 'key', 'dense', 'decoder', 'query'}
    lora_module_names = {'up_proj', 'vision_proj', 'k_proj', 'gate_proj', 'query_proj', 'v_proj', 'o_proj', 'value', 'down_proj', 'self_attn.q_proj', 'key', 'dense', 'decoder', 'query'}
    # lora_module_names = {'up_proj', 'k_proj', 'gate_proj', 'v_proj', 'o_proj', 'value', 'down_proj', 'self_attn.q_proj', 'key', 'dense', 'decoder', 'query'}
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            if role in ["user", "system"]:
                ...
                prompts.append(content.replace(DEFAULT_IMAGE_TOKEN, '').replace('\n', ''))
            else:
                target += encode_id
