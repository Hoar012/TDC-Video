...
def process(line):
    ...
    templ_values = ast.literal_eval(line["templ_values"])
    assert len(templ_values) == len(matches)
    for i in range(len(templ_values)):
        match_pat = matches[i]
        value = templ_values[i]
        question = question.replace(f"<{match_pat}>", value, 1)
    return question
...
def train(args) -> None:
    ...
    for line in tqdm(shard_dataset):
        try:
            ...
            qs = question + " Answer with a single word or phrase."
            q_type = line["type"]
            ans = line["anser"]
    ...
    if global_rank == 0:
        if not os.path.exists("./results/MusicQA"):
            os.mkdir("./results/MusicQA")

        save_time = time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())

        with open(
            os.path.join("./results/MusicQA", f"outputs-{save_time}.json"),
            "w",
        ) as f:
